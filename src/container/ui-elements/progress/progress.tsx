...
import { Animatedata, Coloreddata, CustomProgress2data, Heightdata, Multipleprogress, Stripeddata, progressdata } from "../../../components/common/data/ui-elements/progressdata";
import SpkProgress from "../../../@spk-reusable-components/reusable-uielements/spk-progress";
import { dataprogress4, dataprogress8, progress1, progress10, progress11, progress12, progress13, progress14, progress2, progress3, progress4, progress5, progress6, progress7, progress8, progress9, reusable13, reusableprogress1, reusableprogress12, reusableprogress14, reusableprogress2, reusableprogress3, reusableprogress4, reusableprogress5, reusableprogress6 } from "../../../components/common/data/prism/ui-elements-prism";
			...
			<Row>
				<Col xl={12}>
					<ShowCode title="Progress With Labels" customCardClass="custom-card" customCardBodyClass="" reactCode={progress3} reusableCode={reusableprogress3}>
						{Stripeddata.map((idx) => (
							<SpkProgress key={Math.random()} variant={idx.color} label={`${idx.data}%`} now={idx.data} mainClass="progress mb-3" />
						))}
					</ShowCode>
				</Col>
				<Col xl={12}>
			...
			<Row>
				<Col xl={12}>
					<ShowCode title="Custom Progress-1" customCardClass="custom-card" customCardBodyClass="" reactCode={progress7}>
						...
						<div className="progress progress-sm progress-custom mb-5 progress-animate">
							<h6 className="progress-bar-title bg-secondary">Mobiles</h6>
						...
						<div className="progress progress-sm progress-custom mb-3 progress-animate">
							<h6 className="progress-bar-title bg-success">Mobiles</h6>
			...
			<Row>
				<Col xl={6}>
					<ShowCode title="Gradient Progress" customCardClass="custom-card" customCardBodyClass="" reactCode={progress12} reusableCode={reusableprogress12}>
						{Animatedata.map((idx) => (
							<SpkProgress key={Math.random()} variant={`${idx.color}-gradient`} mainClass={`progress ${idx.class}`} animated={true} now={idx.data} />
						))}
					</ShowCode>
				</Col>
				<Col xl={6}>
					<ShowCode title="Animated Stripped Progress" customCardClass="custom-card" customCardBodyClass="" reactCode={progress13} reusableCode={reusable13}>
						{Animatedata.map((idx) => (
							<SpkProgress key={Math.random()} variant={idx.color} mainClass={`progress ${idx.class}`} animated={true} now={idx.data} />
						))}
					</ShowCode>
				</Col>
				<Col xl={12}>
					<ShowCode title="Custom Animated Progress" customCardClass="custom-card" customCardBodyClass="" reactCode={progress14} reusableCode={reusableprogress14}>
						{Animatedata.map((idx) => (
							<SpkProgress key={Math.random()} variant={`${idx.color}-gradient`} mainClass={`progress progress-animate ${idx.class}`} animated={true} now={idx.data} />
						))}
					</ShowCode>
				</Col>
			</Row>
