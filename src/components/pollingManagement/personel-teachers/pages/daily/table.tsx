...
const STATUS_NAME: Record<StatusId, string> = { 0: 'İzinli', 1: 'Raporlu', 2: 'Görevli', 3: 'Özürsüz' };
const STATUS_CLR: Record<StatusId, string> = { 0: '#009ef7', 1: '#ff55c0', 2: '#18c96e', 3: '#f1416c' };
...
const DailyPollingTable: React.FC = () => {
    ...
    /* —— kullanıcı listesi —— */
    const { usersData = [] } = useUsersTable({ enabled: true, role_id: 2, paginate: 999 });
    ...
    const columns: ColumnDefinition<Row>[] = useMemo(() => [
        ...
        { key: 'absence', label: 'Devamsızlık', render: r => r.absence },
        {
            ...
            render: (_r, _o, idx) => (
                <span
                    style={{ color: STATUS_CLR[rows[idx!].status_id], fontWeight: 500, cursor: 'pointer' }}
                    onClick={() => idx !== undefined && toggleStatus(idx)}
                >
                    {STATUS_NAME[rows[idx!].status_id]}
                </span>
            ),
        },
        {
    ...
    const filters: FilterDefinition[] = useMemo(() => [
        ...
        },
        {
            ...
            options: [{ label: 'Öğretmen', value: 'Öğretmen' }, { label: 'Personel', value: 'Personel' }],
        },
        {
