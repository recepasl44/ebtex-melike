...
export default function SupplierOverviewTab({ supplierId }: SupplierOverviewTabProps) {
  const { supplier, error } = useSupplierShow(supplierId);
  ...
  const columns: ColumnDefinition<Supplier>[] = useMemo(
    () => [
      { key: "season", label: "Sezon", render: (row) => row.season || "" },
      { key: "total_debts", label: "Ödenecek Tutar", render: (row) => row.total_debts || 0 },
      { key: "pay_nakit", label: "Nakit Ödenen", render: (row) => row.pay_nakit || 0 },
      { key: "pay_kredi", label: "KKartı Ödenen", render: (row) => row.pay_kredi || 0 },
      { key: "pay_diger", label: "Diğer Ödenen", render: (row) => row.pay_diger || 0 },
      { key: "total_payments", label: "Toplam Ödenen", render: (row) => row.total_payments || 0 },
      { key: "remaining_debt", label: "Kalan Ödeme", render: (row) => row.remaining_debt || 0 },
    ],
    []
  );
