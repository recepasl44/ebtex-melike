...
interface PlanCalendarFilterProps {
  PlanCalendarChange: (data: { id: string; name: string }) => void;
}
const PlanCalendarFilter: React.FC<PlanCalendarFilterProps> = ({
  PlanCalendarChange,
}) => {
  ...
  useEffect(() => {
    PlanCalendarChange({ id: studentId, name: inputName });
  }, [studentId, inputName, PlanCalendarChange]);
  ...
  const updateQueryParams = (params: QueryParams) => {
    ...
    if (params.start_date) {
      query.set("start_date", String(params.start_date)); // Tarihi URL'e ekleyin
    }
    if (params.end_date) {
      query.set("end_date", String(params.end_date)); // Tarihi URL'e ekleyin
    }
  ...
  const handleDateSelect = (date: Date) => {
    ...
    localStorage.setItem("startDate", format(date, "yyyy-MM-dd"));
    localStorage.setItem("endDate", format(date, "yyyy-MM-dd"));
  };
...
export default PlanCalendarFilter;
