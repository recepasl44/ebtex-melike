...
import { homeworkNumbersTable } from "./Table";
...
interface HomeworkNumbersFilterProps {
  HomeworkNumbersChange: (data: { id: string; name: string }) => void;
}

const HomeworkNumbersFilter: React.FC<HomeworkNumbersFilterProps> = ({
  HomeworkNumbersChange,
}) => {
  const columns = homeworkNumbersTable();
  ...
  useEffect(() => {
    HomeworkNumbersChange({ id: studentId, name: inputName });
  }, [studentId, inputName, HomeworkNumbersChange]);
    ...
    <div>
      ...
      {debouncedName ? (
        ...
        <div className="alert alert-info mt-3">
          Lütfen ödev sayılarını görmek için bir öğrenci seçiniz.
        </div>
      )}
    </div>
  );
...
export default HomeworkNumbersFilter;
