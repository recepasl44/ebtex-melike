...
const FlexibleAccordionTable: React.FC<FlexibleAccordionTableProps> = ({
  ...
  const getCellStyle = (isHeader?: boolean, style?: React.CSSProperties): React.CSSProperties => ({
  ...
  const CellContent: React.FC<{ cell: TableCell; isOpen?: boolean; rowId: string; cellIndex: any }> = ({
    cell,
    isOpen,
    rowId,
    cellIndex,
  ...
  const renderCell = (cell: TableCell, i: number, isOpen?: boolean, rowId?: string) => {
    const props = {
    ...
    const content = <CellContent cell={cell} isOpen={isOpen} rowId={rowId || ""} cellIndex={i} />;
  ...
  const renderRowRecursive = (row: TableRow): React.ReactNode => {
    ...
    return (
      <React.Fragment key={row.id}>
        ...
        {isAccordion && isOpen && row.children?.map((child) => renderRowRecursive(child))}
      </React.Fragment>
    );
  };
