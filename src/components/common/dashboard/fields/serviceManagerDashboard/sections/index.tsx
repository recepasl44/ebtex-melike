...
const Row10Component: React.FC<Row10Props> = ({ data }) => {
  ...
  const serviceUsageByDayOfTheWeek =
    firstItem?.service_usage_by_day_of_the_week || [];
  // Servis Araç Bilgileri
  const serviceVehicleInformation =
    firstItem?.service_vehicle_information || [];
  ...
  const servicePaymentInformation =
    firstItem?.service_payment_information || [];
    // Ücret Durumu
  const wageStatus = firstItem?.wage_status;
  return (
    <Row>
      {/* Sol Sütun - Col 9 */}
      <Col xl={9} xxl={9}>
