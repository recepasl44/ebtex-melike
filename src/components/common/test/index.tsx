...
import { useSchoolTable } from '../../hooks/schools/useSchoolList';
...
export default function QuestionLabeling() {
  ...
  const filtersState = useMemo(
    ...
    [branch, appointment_type, appointment_time, program_id, Class_level, first_name, student_id, pageSize]
  );
  ...
  const appointmentParams = useMemo(() => ({
    ...
    per_page: pageSize,
  }), [pageSize, page, branch, appointment_type, appointment_time, Class_level, student_id]);
  ...
  const filters = useMemo(() => {
    const basicFilters = [
      ...
      },
      {
        ...
        onChange: (val: string) => {
          if (val) {
            const matchedStudent = data?.find((item) =>
            ...
            if (matchedStudent) {
              ...
              setStudent_id(String(matchedStudent.id));
    ...
    branch, appointment_type, appointment_time, program_id, Class_level, first_name,
    branchData, programsData, levelsData, first_name, student_id
  ]);
  ...
  const columns: ColumnDefinition<data>[] = useMemo(() => [
...

    {
      ...
      render: (row) => row.student?.first_name ?? '-'
    },
    {
      key: 'last_name',
      label: 'Soyadı',
      render: (row) => row.student?.last_name ?? '-'
    },
    {
      key: 'level',
...

    {
      ...
      label: 'Ücret',
      render: (row) => {
        const enroll = row.student?.enrollments?.[0];
        return enroll?.final_fee ?? enroll?.total_fee ?? '-';
      }
    },
...

    {
      ...
      render: (row, openDeleteModal) => (
        <>
          <Button variant="primary" size="sm" onClick={() => navigate(`/testcrud/${row.id}`)}>
            Details
          </Button>
