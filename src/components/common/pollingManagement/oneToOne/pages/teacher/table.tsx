...
export default function TeacherOneByOnePlanTable() {
    ...
    const filters: FilterDefinition[] = useMemo(() => [
                {
                    key: 'dateRange', label: 'Tarih Aralığı', type: 'doubledate' as const, col: 1,
                    value: dateRange,
                    onChange: (v: { startDate: string; endDate: string }) => setDateRange(v ?? { startDate: '', endDate: '' }),
                },
                {
                    key: 'week_days', label: 'Haftanın Günleri', type: 'multiselect', col: 1,
                    value: weekDays.join(','), // Convert array to comma-separated string
                ...
                },
                {
                    ...
                    value: lesson.join(','), // Convert array to comma-separated string
                    onClick: () => setEnabled(e => ({ ...e, lessons: true })),
                    onChange: (v: string) => setLesson(v ? v.split(',') : []),
                    options: lessonsData.map((l: LessonData) => ({ value: String(l.id), label: l.name })),
                },
                {
                    key: 'teacher', label: 'Öğretmen', type: 'multiselect', col: 1,
                    value: teacher.join(','), // Convert array to comma-separated string
                    onClick: () => setEnabled(e => ({ ...e, teachers: true })),
                    onChange: (v: string) => setTeacher(v ? v.split(',') : []),
                    options: teachersData.map((t: TeacherData) => ({
                ...
                },
                {
                    ...
                    onChange: setAreaId,
                    options: usedAreasData.map((a: UsedAreaData) => ({ value: String(a.id), label: a.name })),
                },
            ], [
