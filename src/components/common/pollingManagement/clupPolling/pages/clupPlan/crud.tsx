...
export default function ClubPlanModal({
    ...
    const [levelId] = useState<number | null>(null);
    const [enabled, setEnabled] = useState({
        groups: false, areas: false, levels: false, classes: false,
        users: false, teachers: false, students: false,
    });
    ...
    const { attendanceStudentsData: studentsData = [] } =
        useAttendanceStudentsTable({ enabled: enabled.students, level_id: levelId ?? undefined });
    ...
    useEffect(() => {
        if (mode === 'update' && fetched) {
            ...
            setEnabled(e => ({
                ...
                users: true, teachers: true, students: true,
            }));
        }
    }, [mode, fetched]);
    ...
    const fields: FieldDefinition[] = useMemo(() => [
        ...
        {
            name: 'week_days', label: 'Haftanın Günleri', type: 'select',
            options: [
    ...
    ], [groupsData, usedAreasData, levelsData, classroomData, managersData, teachersData, studentsData]);
