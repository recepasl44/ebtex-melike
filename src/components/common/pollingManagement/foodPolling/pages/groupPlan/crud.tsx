/* -------------------------------------------------------------------------- */
/*  src/components/common/pollingManagement/foodPolling/pages/groupPlan/      */
/*  FoodPlanModal.tsx                                                         */
/*                                                                            */
/*  “Yemek Yoklama › Grup Planla” – Ekle / Düzenle (Mod-al)                    */
/* -------------------------------------------------------------------------- */
...
interface IForm extends FormikValues {
    ...
    manager_ids: number[];          /* Görevli yöneticiler (users)         */
    teacher_ids: number[];          /* Görevli öğretmenler                 */
}
...
const FoodPlanModal: React.FC<ModalProps> = ({ show, onClose, onRefresh }) => {
    ...
    /* ------------------ seçimlere bağlı “lazy load” bayrakları ------------ */
    const [levelId, setLevelId] = useState<number | null>(null);
    const [enabled, setEnabled] = useState({
        groups: true,              /* öğün / grup her zaman geliyor         */
        areas: true,              /* yemek alanı                            */
        levels: true,
        classes: false,
        students: false,
        teachers: false,
        users: false,            /* sorumlu & yönetici listesi            */
    });
    ...
    /* role_id 2 → şimdilik hem “Yemekhane Sorumlusu” hem “Görevli Yönetici” */
    ...
    const { addNewAttendance,
        status: addSt,
        error: addErr } = useAttendanceAdd();

    const { updateExistingAttendance,
        status: updSt,
        error: updErr } = useAttendanceUpdate();
