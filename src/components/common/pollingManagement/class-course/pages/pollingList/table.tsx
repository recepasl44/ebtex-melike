...
export default function PollingListTable() {
    ...
    const filters: FilterDefinition[] = useMemo(() => [
        {
            key: 'dateRange', label: 'Tarih Aralığı', type: 'doubledate' as const, col: 2,
            value: dateRange,
            onChange: v => setDateRange(v ?? { startDate: '', endDate: '' }),
        },
        {
            key: 'class_level', label: 'Sınıf Seviyesi', type: 'select' as const, col: 1,
            value: classLevel,
        ...
        {
            key: 'classroom', label: 'Sınıf / Şube', type: 'select' as const, col: 1,
            value: classroom,
        ...
        {
            key: 'lesson', label: 'Ders', type: 'select' as const, col: 1,
            value: lessons.join(','),
            dependencyKey: 'class_level',
            onClick: () => setEnabled(e => ({ ...e, lessons: true })),
            onChange: (val: string) => setLessons(val ? [val] : []),
            options: lessonsData.map(l => ({ value: String(l.id), label: l.name })),
        },
        {
            key: 'student', label: 'Öğrenci', type: 'select' as const, col: 1,
            value: student,
