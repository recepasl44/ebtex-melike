...
export default function PollingCountsTable() {
    ...
    const filters: FilterDefinition[] = useMemo(() => [
        ...
        {
            key: 'lesson', label: 'Ders', type: 'select', col: 1, // changed from multiselect to select
            dependencyKey: 'class_level',
            value: lessons.join(','), // convert array to comma-separated string
            onClick: () => setEnabled(e => ({ ...e, lessons: true })),
            onChange: (v: string) => setLessons(v ? v.split(',') : []), // convert string back to array
