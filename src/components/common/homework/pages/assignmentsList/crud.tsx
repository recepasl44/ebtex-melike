...
const AssignmentsListCrudModal: React.FC<Props> = ({
    show,
    onClose,
    onRefresh,
}) => {
    ...
    const { data: studentsData = [] } = useListStudents({ enabled: enabled.student });
    ...
    const studentOpts = useMemo(
        () => studentsData.map((s) => ({ value: s.id, label: `${s.name} ${s.surname}` })),
        [studentsData],
    );
    ...
    useEffect(() => {
        if (mode === 'update' && assignmentStudent && !Array.isArray(assignmentStudent)) {
            setInitial({
                ...
                student_file: null,
                delivery_format: (assignmentStudent.delivery_format === 'online' || assignmentStudent.delivery_format === 'face-to-face') ? assignmentStudent.delivery_format : 'online',
            });
    ...
    const handleSubmit = async (vals: IForm, _h: FormikHelpers<IForm>) => {
        if (mode === 'add') await addNewAssignmentStudent({
            ...
            assigment_id: 0 // Provide a default value for assigment_id
        });
        else if (id)
    ...
    return (
        <ReusableModalForm<IForm>
            show={show}
            title={mode === 'add' ? 'Yeni Ödev Ekle' : 'Ödev Düzenle'}
            fields={getFields}
...
export default AssignmentsListCrudModal;
