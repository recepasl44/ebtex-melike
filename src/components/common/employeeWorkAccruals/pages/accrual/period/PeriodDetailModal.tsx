...
import { EmployeeEarningsPeriodItem } from '../../../../../../types/employeeEarningsPeriod/list'
...
export default function PeriodDetailModal({ row, onClose }: Props) {
    const year = row.period?.split('-')[0] || new Date().getFullYear()
    const period = row.period

    const items: EmployeeEarningsPeriodItem[] = INCOME_TYPES.map((t) => {
        const existing = row.items?.find((i: EmployeeEarningsPeriodItem) => i.income_type === t.key)
        return {
            id: existing?.id || 0,
    ...
    const columns: ColumnDefinition<EmployeeEarningsPeriodItem>[] = [
    ...
    return (
        <Modal show centered onHide={onClose}>
            ...
            <Modal.Body>
                <ReusableTable<EmployeeEarningsPeriodItem>
                    tableMode='single'
                    columns={columns}
                    data={items}
                    loading={false}
