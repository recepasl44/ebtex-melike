    ...
    public function render($request, Exception|\Throwable $exception)
    {
        if (strpos($request->url(), '/api/') !== false) {
            ...
            if ($exception instanceof MethodNotAllowedHttpException) {
                return $this->setStatusCode(405)->respondWithError('Please check HTTP Request Method. - MethodNotAllowedHttpException: '. $exception->getMessage());
            }

            if ($exception instanceof NotFoundHttpException) {
                return $this->setStatusCode(404)->respondWithError('Please check your URL to make sure request is formatted properly. - NotFoundHttpException: '. $exception->getMessage());
            }

            if ($exception instanceof GeneralException) {
                return $this->setStatusCode(400)->respondWithError($exception->getMessage());
            }
            ...
            */
            ...
            // Tüm diğer istisnalar
            return response()->json([
                'success' => false,
                'message' => env('APP_DEBUG') ? $exception->getMessage() : 'An unexpected error occurred. Please try again later.',
                'error_code' => $exception->getCode() ?: 500,
                'error' => env('APP_DEBUG') ? $exception->getTrace() : [],
            ], 500);
