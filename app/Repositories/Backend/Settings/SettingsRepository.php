...
class SettingsRepository extends BaseRepository
{
    ...
    public function update(Setting $setting, array $input)
    {
        ...
        if (!empty($input['logo_circle'])) {
            $this->removeLogo($setting, 'logo_circle');

            $input['logo_circle'] = $this->uploadLogo($setting, $input['logo_circle'], 'logo');
        }

        if (!empty($input['logo_reverse'])) {
            $this->removeLogo($setting, 'logo_reverse');

            $input['logo_reverse'] = $this->uploadLogo($setting, $input['logo_reverse'], 'logo');
        }
        ...
        throw new GeneralException(_tr('exceptions.backend.settings.update_error'));
    }
