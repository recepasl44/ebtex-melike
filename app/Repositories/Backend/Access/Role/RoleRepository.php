...
class RoleRepository extends BaseRepository
{
    ...
    public function getForDataTable()
    {
        return $this->query()
            ...
            ->where(config('access.roles_table').'.platform_id', 1)
    ...
    public function create(array $input)
    {
        if ($this->query()->where('name', $input['name'])->first()) {
            throw new GeneralException(_tr('exceptions.backend.access.roles.already_exists'));
        }
        ...
        if (!$all) {
            ...
            if (config('access.roles.role_must_contain_permission') && count($input['permissions']) == 0) {
                throw new GeneralException(_tr('exceptions.backend.access.roles.needs_permission'));
            }
        }
        ...
        DB::transaction(function () use ($input, $all) {
            ...
            throw new GeneralException(_tr('exceptions.backend.access.roles.create_error'));
        });
    }
    ...
    public function update($role, array $input)
    {
        ...
        DB::transaction(function () use ($role, $input, $all) {
            ...
            throw new GeneralException(_tr('exceptions.backend.access.roles.update_error'));
        });
    }
    ...
    public function delete(Role $role)
    {
        ...
        if ($role->id == 1) { //id is 1 because of the seeder
            throw new GeneralException(_tr('exceptions.backend.access.roles.cant_delete_admin'));
        }

        //Don't delete the role is there are users associated
        if ($role->users()->count() > 0) {
            throw new GeneralException(_tr('exceptions.backend.access.roles.has_users'));
        }
        ...
        DB::transaction(function () use ($role) {
            ...
            throw new GeneralException(_tr('exceptions.backend.access.roles.delete_error'));
        });
    }
