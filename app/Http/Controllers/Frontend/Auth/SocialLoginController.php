...
class SocialLoginController extends Controller
{
    ...
    public function login(Request $request, $provider)
    {
        ...
        if (!in_array($provider, $this->helper->getAcceptedProviders())) {
            return redirect()->route('frontend.index')->withFlashDanger(_tr('auth.socialite.unacceptable', ['provider' => $provider]));
        }
        ...
        if (is_null($user) || !isset($user)) {
            return redirect()->route('frontend.index')->withFlashDanger(_tr('exceptions.frontend.auth.unknown'));
        }

        // Check to see if they are active.
        if (!$user->isActive()) {
            throw new GeneralException(_tr('exceptions.frontend.auth.deactivated'));
        }
