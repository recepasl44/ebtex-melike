...
class UserStatusController extends Controller
{
    ...
    public function mark(User $user, $status, EditUserRequest $request)
    {
        ...
        return redirect()->route($status == 1 ? 'admin.access.user.index' : 'admin.access.user.deactivated')->withFlashSuccess(_tr('alerts.backend.users.updated'));
    }
    ...
    public function delete(User $deletedUser, DeleteUserRequest $request)
    {
        ...
        return redirect()->route('admin.access.user.deleted')->withFlashSuccess(_tr('alerts.backend.users.deleted_permanently'));
    }
    ...
    public function restore(User $deletedUser, DeleteUserRequest $request)
    {
        ...
        return redirect()->route('admin.access.user.index')->withFlashSuccess(_tr('alerts.backend.users.restored'));
    }
}
