    ...
    public function handle($request, Closure $next)
    {
        ...
        if (!Cookie::has($remember_cookie) && config('session.timeout_status')) {
            ...
            if (!session('lastActivityTime')) {
                ...
                return redirect()->route('frontend.auth.login')->withFlashWarning(_tr('strings.backend.general.timeout').$this->timeout / 60 ._tr('strings.backend.general.minutes'))->withInput(compact('email'))->withCookie($cookie);
            }
